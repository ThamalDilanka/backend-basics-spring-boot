            ...
            <Card key={plant.id} className="flex flex-col justify-between">     
              <CardHeader className="flex flex-row items-center gap-4">
                <div className="w-16 h-16 rounded-full border-4 border-dashed border-green-500 flex items-center justify-center bg-green-50 shrink-0 shadow-sm hover:rotate-12 transition-transform duration-300">
                  <img src={plant.imageUrl || "/plant.png"} alt={plant.name} className="w-full h-full object-cover rounded-full hover:scale-110 transition-transform duration-300" />
                </div>
                <div>
              ...
              <CardContent>
                ...
                <div className="flex items-center gap-4 mt-4">
                  <div className="flex flex-col">
                    <span className="inline-block mt-1 px-3 py-1 bg-slate-100 text-slate-700 text-sm rounded-full">Status: {plant.status}</span>
                  </div>
                  <div className="ml-auto flex items-center gap-2">
                    <img src={plant.ownerImageUrl || "https://ui-avatars.com/api/?name=" + (plant.ownerName || "User") + "&background=random"} alt={plant.ownerName} className="w-8 h-8 rounded-full border border-slate-200" />
                    <span className="text-sm text-slate-600 font-medium">{plant.ownerName || "Unknown User"}</span>
                  </div>
                </div>
              </CardContent>
              <CardFooter className="inline-block ">
                {plant.ownerId === loggedInMemberId ? (
                  <Button className="w-full mb-2" disabled variant="secondary">Your Plant</Button>
                ) : (
                  <Button 
                    className="w-full mb-2 bg-green-700 hover:bg-green-800" 
                ...
                <Link to={"/plant/" + plant.id} className="w-full">
                  <Button variant="outline" className="w-full border-green-600 text-green-700 hover:bg-green-50">View Details</Button>
                </Link>
              </CardFooter>
            </Card>
          ))}
